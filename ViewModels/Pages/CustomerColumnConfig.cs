...
public partial class CustomerColumnConfig : ObservableObject
{
    private static readonly string _filePath =
        Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "customer_columns.json");
    ...
    [ObservableProperty] private bool _showCreditLimit    = false;
    [ObservableProperty] private bool _showRole           = true;
    [ObservableProperty] private bool _showNotes          = false;

    public static CustomerColumnConfig Load()
    {
        try { if (File.Exists(_filePath)) return JsonSerializer.Deserialize<CustomerColumnConfig>(File.ReadAllText(_filePath)) ?? new(); } catch { }
        return new();
    }
    public void Save()
    {
